...
class DocumentProcessor:
    ...
    def load_document(file_path: str, chunk_size: int = CHUNK_SIZE) -> list:
        ...
        try:
            ...
            chunks = DocumentProcessor._split_text_with_langchain(content, chunk_size)
    ...
    def load_document_from_memory(file_bytes: bytes, file_extension: str, chunk_size: int = CHUNK_SIZE) -> list:
        ...
        try:
            ...
            # Use LangChain's optimized text splitter
            chunks = DocumentProcessor._split_text_with_langchain(content, chunk_size)
            print(f"✅ Processed document and split into {len(chunks)} chunks using LangChain splitter")
            return chunks
        except Exception as e:
    ...
    @staticmethod
    def _split_text_with_langchain(text: str, chunk_size: int = CHUNK_SIZE) -> list:
        """
        Use LangChain's RecursiveCharacterTextSplitter for optimized text chunking.
